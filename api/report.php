<?php
// Read-only эндпоинт для сводного дашборда портала (ТЗ, лист 7, чек-лист
// п.4: портал получает данные проекта только отсюда, прямого доступа к БД у
// портала нет). Отдаёт только обезличенные агрегаты — никаких имён, логинов,
// рецептур и журналов. ТРЕБУЕТ авторизации: блок 'pulse' — это уже
// конкретные бизнес-показатели производства (тонны сварено, загрузка
// реакторов, ассортимент SKU), а не просто счётчики, поэтому наружу без
// пароля не отдаём — тот же принцип, что и в Tracker api/report.php.
//
...
apiHeaders('GET'); // ALLOWED_ORIGIN по умолчанию (не '*') — требует авторизацию ниже
header('Cache-Control: private, max-age=60');

requireAuth(); // любая валидная сессия — локальный логин или SSO-токен портала (только admin)
