...
- ✅ **Уровень 2 «Реакторы»** — **задеплоен, на боевом** (Pages был завис на `65ebc8c`, расшевелили через `gh api -X POST .../pages/builds` → сборка `5f8739a` прошла). Проверить: `curl .../index.html | grep dash-reactor-cards`.
...
- ✅ **Уровень 3 «Продукты»** — **задеплоен, на боевом** (HEAD `29d5150`, Pages build `built`; на сайте есть `renderProducts`/`bi-products`). Проверен на изолированном Node-харнессе (11/11 + пустой кейс).
...
- ✅ **Тренды (ТЗ §10 п.9)** — **задеплоен, на боевом** (HEAD `cfd591a`; на сайте есть `renderMonthTrend`/`bi-month-trend`). Проверен на изолированном харнессе (месяц+данные / пустой / скрытие вне месяца / end-to-end, 0 ошибок).
